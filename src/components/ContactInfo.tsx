    ...
    <section className="relative bg-gradient-to-r from-slate-900 via-blue-900 to-slate-900 text-white py-12 sm:py-16 lg:py-20">
      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-8 sm:mb-12">
          ...
          <p className="text-lg sm:text-xl text-blue-200 font-light">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 max-w-4xl mx-auto">
          {contactItems.map((item, index) => {
            ...
            const content = (
              <div className="text-center p-4 sm:p-6 bg-white/10 backdrop-blur-sm rounded-xl border border-white/20 hover:bg-white/20 transition-all duration-300 group">
                <div className="w-12 h-12 sm:w-16 sm:h-16 bg-gradient-to-br from-blue-500 to-cyan-500 rounded-full flex items-center justify-center mx-auto mb-3 sm:mb-4 group-hover:scale-110 transition-transform">
                  <Icon className="w-6 h-6 sm:w-8 sm:h-8 text-white" />
                </div>
                <p className="text-xs sm:text-sm text-blue-200 uppercase tracking-wide mb-2">{item.label}</p>
                <p className="text-sm sm:text-lg font-medium text-white break-words">{item.value}</p>
              </div>
            );
